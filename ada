...
local MenuName = isfile("luxuryhub/menuname.txt") and readfile("luxuryhub/menuname.txt") or nil
...
if not isfolder("luxuryhub") then
	makefolder("luxuryhub")
end

do -- Files
	if not isfile("luxuryhub/relations.bb") then
		writefile("luxuryhub/relations.bb", "friends:\npriority:")
	end

    if not isfile("luxuryhub/chatspam.bb") then
        writefile("luxuryhub/chatspam.bb",
            [[BEACH BOAT ON TOP
...
beach boat does not miss]])
    ...
    if not isfile("luxuryhub/killsay.bb") then
        writefile("luxuryhub/killsay.bb",
...
[name] get better]])
    ...
    if not isfolder("luxuryhub/" .. menu.game) then
        makefolder("luxuryhub/" .. menu.game)
    end

    if not isfile("luxuryhub/" .. menu.game .. "/Default.bb") then
        writefile("luxuryhub/" .. menu.game .. "/Default.bb", "")
    end
end

do
    local customtxt = readfile("luxuryhub/chatspam.bb")
    for s in customtxt:gmatch("[^\n]+") do
        table.insert(customChatSpam, s)
    end

    customtxt = readfile("luxuryhub/killsay.bb")
    for s in customtxt:gmatch("[^\n]+") do
...
local function GetConfigs()
	local result = {}
	local directory = "luxuryhub\\" .. menu.game
...
local function UnpackRelations()
	local str = isfile("luxuryhub/relations.bb") and readfile("luxuryhub/relations.bb") or nil
	local final = {
		friends = {},
		priority = {},
	}
...
local function WriteRelations()
	...
	writefile("luxuryhub/relations.bb", str)
end
CreateThread(function()
...
function menu.Initialize(menutable)
	...
	function menu.saveconfig()
		...
		writefile("luxuryhub/".. menu.game.. "/".. menu.options["Settings"]["Configuration"]["ConfigName"][1].. ".bb", figgy)
		CreateNotification('Saved "' .. menu.options["Settings"]["Configuration"]["ConfigName"][1] .. '.bb"!')
		UpdateConfigs()
	end

	function menu.loadconfig()
		local configname = "luxuryhub/".. menu.game.. "/".. menu.options["Settings"]["Configuration"]["ConfigName"][1].. ".bb"
	...
	local function buttonpressed(bp)
		...
		elseif bp == menu.options["Settings"]["Configuration"]["Delete Config"] then
			delfile("luxuryhub/".. menu.game.. "/".. menu.options["Settings"]["Configuration"]["ConfigName"][1].. ".bb")
							...
							[1] = {
								content = {
									...
									},
									{
										...
										tooltip = "Spams chat, Custom options are located in the luxuryhub/chatspam.bb",
									},
									{
										type = TOGGLE,
